...
including inline refresh progress and failure details. When no source is
configured, the served dashboard can collect a GW2 API key in a local setup page
and keep it in memory for that app session.
